...
export function getMemberIds(registry) {
...
export function getAllMembers(registry) {
  if (!isValidRegistry(registry)) {
    return [];
  }
...
export function getMembershipEntries(registry) {
  if (!isValidRegistry(registry)) {
    return [];
  }
...
export function hasMemberId(registry, memberId) {
  if (!isValidRegistry(registry) || typeof memberId !== "string") {
    return false;
  }

  return registry.hasOwnProperty(memberId);
}

export function removeMemberId(registry, memberId) {
  if (!isValidRegistry(registry) || typeof memberId !== "string") {
    return false;
  }

  if (!registry.hasOwnProperty(memberId)) {
    return false;
  }

  delete registry[memberId];
  return true;
}
