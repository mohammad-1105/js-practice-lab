import { generateTransitPass } from '../src/05-transit-pass.js';
...
describe('05 - Transit Pass Generator (8 pts)', () => {
  ...
  describe('Basic pass generation', () => {
    test('First class pass generated correctly', () => {
      const result = generateTransitPass({
        name: 'alex parker', from: 'midtown', to: 'downtown', classType: 'first'
      });
      expect(result).toBe(
        'CITY TRANSIT PASS\n---\nName: ALEX PARKER\nFrom: Midtown\nTo: Downtown\nClass: FIRST\nPass ID: FMIDDOW'
      );
    ...
    test('Second class pass generated correctly', () => {
      const result = generateTransitPass({
        name: 'taylor morgan', from: 'union', to: 'uptown', classType: 'second'
      });
      expect(result).toBe(
        'CITY TRANSIT PASS\n---\nName: TAYLOR MORGAN\nFrom: Union\nTo: Uptown\nClass: SECOND\nPass ID: SUNIUPT'
      );
    ...
    test('Single word name', () => {
      const result = generateTransitPass({
        name: 'ryan', from: 'lakeside', to: 'riverside', classType: 'first'
      });
  ...
  describe('Name formatting', () => {
    test('Name is converted to uppercase', () => {
      const result = generateTransitPass({
        name: 'chris lane', from: 'midtown', to: 'union', classType: 'first'
      });
  ...
  describe('Station Title Case', () => {
    test('From and To are in Title Case', () => {
      const result = generateTransitPass({
        name: 'test', from: 'MIDTOWN', to: 'downtown', classType: 'first'
      });
  ...
  describe('Pass ID generation', () => {
    test('Pass ID uses first char of classType + first 3 of from + first 3 of to', () => {
      const result = generateTransitPass({
        name: 'test', from: 'chicago', to: 'phoenix', classType: 'second'
      });
  ...
  describe('Case insensitivity for classType', () => {
    test('"FIRST" works same as "first"', () => {
      const result = generateTransitPass({
        name: 'test', from: 'midtown', to: 'union', classType: 'FIRST'
      });
    ...
    test('"Second" works same as "second"', () => {
      const result = generateTransitPass({
        name: 'test', from: 'midtown', to: 'union', classType: 'Second'
      });
  ...
  describe('Validation', () => {
    test('null input returns INVALID PASS', () => {
      expect(generateTransitPass(null)).toBe('INVALID PASS');
    });
    ...
    test('Missing name returns INVALID PASS', () => {
      expect(generateTransitPass({ from: 'midtown', to: 'union', classType: 'first' }))
        .toBe('INVALID PASS');
    });

    test('Empty name returns INVALID PASS', () => {
      expect(generateTransitPass({ name: '', from: 'midtown', to: 'union', classType: 'first' }))
        .toBe('INVALID PASS');
    });

    test('Missing from returns INVALID PASS', () => {
      expect(generateTransitPass({ name: 'test', to: 'union', classType: 'first' }))
        .toBe('INVALID PASS');
    });

    test('Invalid classType returns INVALID PASS', () => {
      expect(generateTransitPass({ name: 'test', from: 'midtown', to: 'union', classType: 'third' }))
        .toBe('INVALID PASS');
    });

    test('undefined input returns INVALID PASS', () => {
      expect(generateTransitPass(undefined)).toBe('INVALID PASS');
    });
  });
});
