import { processTrainBooking } from '../src/12-train-booking-status.js';
...
describe('12 - Train Booking Status Processor (9 pts)', () => {
  ...
  describe('Train info template literal', () => {
    test('trainInfo formatted correctly', () => {
      const result = processTrainBooking(baseBooking);
      expect(result.trainInfo).toBe('Train: 1201 - Northeast Express | NYP -> BOS | Class: Business');
    });
  });
  ...
  describe('Summary statistics', () => {
    test('All confirmed passengers', () => {
      ...
      expect(result.summary).toEqual({
        totalPassengers: 2, confirmed: 2, waitlisted: 0, cancelled: 0, standby: 0,
        allConfirmed: true, anyWaitlisted: false
      });
    });
  ...
  describe('Validation', () => {
    test('null bookingData returns null', () => {
      expect(processTrainBooking(null)).toBeNull();
    });
