import { formatCoffeeMenu } from '../src/02-coffee-shop-menu.js';

describe('02 - Coffee Shop Menu (7 pts)', () => {

  describe('Basic formatting', () => {
    test('Single item formatted correctly', () => {
      expect(formatCoffeeMenu([{ name: 'latte', price: 15 }]))
        .toBe('LATTE - $15');
    });

    test('Two items joined with pipe separator', () => {
      expect(formatCoffeeMenu([
        { name: 'latte', price: 15 },
  ...
  describe('Name transformations', () => {
    test('Already uppercase name stays uppercase', () => {
      expect(formatCoffeeMenu([{ name: 'COFFEE', price: 10 }]))
        .toBe('COFFEE - $10');
    });

    test('Mixed case becomes full uppercase', () => {
      expect(formatCoffeeMenu([{ name: 'Iced Latte', price: 15 }]))
        .toBe('ICED LATTE - $15');
    });
  });
  ...
  describe('Validation', () => {
    ...
    test('null returns empty string', () => {
      expect(formatCoffeeMenu(null)).toBe('');
    });

    test('undefined returns empty string', () => {
      expect(formatCoffeeMenu(undefined)).toBe('');
    });
  });
});
