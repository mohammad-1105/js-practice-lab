import { formatMovieTitle } from '../src/03-movie-title-formatter.js';

describe('03 - Movie Title Formatter (8 pts)', () => {

  describe('Basic title casing', () => {
    test('All lowercase to Title Case', () => {
      expect(formatMovieTitle('the social network'))
        .toBe('The Social Network');
    });

    test('ALL CAPS to Title Case', () => {
      expect(formatMovieTitle('INCEPTION'))
        .toBe('Inception');
    });

    test('Mixed case to Title Case', () => {
      expect(formatMovieTitle('the dARK kNIGHT rISES'))
        .toBe('The Dark Knight Rises');
    });
  });
  ...
  describe('Small words handling', () => {
    test('"of" stays lowercase in middle', () => {
      expect(formatMovieTitle('star of the sea'))
        .toBe('Star of the Sea');
    });

    test('"and" stays lowercase in middle', () => {
      expect(formatMovieTitle('war and peace'))
        .toBe('War and Peace');
    });

    test('"the" stays lowercase in middle', () => {
      expect(formatMovieTitle('lord of the rings'))
        .toBe('Lord of the Rings');
    });

    test('Small word at beginning is capitalized', () => {
      expect(formatMovieTitle('the legend of tomorrow'))
        .toBe('The Legend of Tomorrow');
    });

    test('"a" and "an" stay lowercase in middle', () => {
      expect(formatMovieTitle('once upon a time in an office'))
        .toBe('Once Upon a Time in an Office');
    });

    test('Multiple small words in a title', () => {
      expect(formatMovieTitle('a story of code and coffee'))
        .toBe('A Story of Code and Coffee');
    });
  });
  ...
  describe('Validation', () => {
    ...
    test('null returns empty string', () => {
      expect(formatMovieTitle(null)).toBe('');
    });

    test('undefined returns empty string', () => {
      expect(formatMovieTitle(undefined)).toBe('');
    });
  });
});
