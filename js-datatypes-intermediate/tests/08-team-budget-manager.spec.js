import { summarizeTeamBudget } from '../src/08-team-budget-manager.js';

describe('08 - Team Budget Manager (8 pts)', () => {

  describe('Basic calculations', () => {
    test('Total spent is correct sum of prices', () => {
      const result = summarizeTeamBudget(
        { name: 'BOS', budget: 9000 },
        [{ name: 'Carter', role: 'offense', price: 1200 }, { name: 'Brooks', role: 'defense', price: 1600 }]
      );
    ...
    test('playerCount matches array length', () => {
      const result = summarizeTeamBudget(
        { name: 'BOS', budget: 9000 },
        [{ name: 'A', role: 'offense', price: 100 }, { name: 'B', role: 'defense', price: 200 }]
      );
    ...
    test('averagePrice is correctly rounded', () => {
      const result = summarizeTeamBudget(
        { name: 'BOS', budget: 9000 },
        [{ name: 'A', role: 'offense', price: 100 }, { name: 'B', role: 'defense', price: 201 }]
      );
  ...
  describe('Validation', () => {
    ...
    test('Negative budget returns null', () => {
      expect(summarizeTeamBudget({ name: 'BOS', budget: -100 }, [{ name: 'A', role: 'offense', price: 100 }])).toBeNull();
    });

    test('Missing budget returns null', () => {
      expect(summarizeTeamBudget({ name: 'BOS' }, [{ name: 'A', role: 'offense', price: 100 }])).toBeNull();
    });
  });
});
