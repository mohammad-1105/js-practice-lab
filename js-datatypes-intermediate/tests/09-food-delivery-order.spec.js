import { buildDeliveryOrder } from '../src/09-food-delivery-order.js';
...
describe('09 - Food Delivery Order Builder (9 pts)', () => {
  ...
  describe('Item calculation', () => {
    test('Single item, no addons', () => {
      ...
      expect(result.items[0]).toEqual({
        name: "Burger Bowl", qty: 1, basePrice: 150, addonTotal: 0, itemTotal: 150
      });
    });
    ...
    test('Single item with addons', () => {
      ...
      expect(result.items[0]).toEqual({
        name: "Chicken Wrap", qty: 1, basePrice: 300, addonTotal: 50, itemTotal: 350
      });
    });
  ...
  describe('Validation', () => {
    ...
    test('null cart returns null', () => {
      expect(buildDeliveryOrder(null)).toBeNull();
    });
  });
});
